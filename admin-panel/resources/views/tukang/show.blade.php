...
<div class="page-content">
    ...
    <div class="px-4 pt-4">
        <div class="flex items-start justify-between">
            <div>
                <h1 class="text-xl font-bold text-navy">{{ $tukang->nama }}</h1>
                ...
                <div class="flex items-center gap-1 mt-1">
                    ...
                    <span class="text-sm text-gray-500">{{ $tukang->lokasi ?? 'Surakarta' }}</span>
                </div>
            </div>
            <span class="px-3 py-1 rounded-full text-xs font-bold {{ $tukang->status_aktif ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500' }}">
        ...
        <div class="mt-4">
            ...
            <p class="text-sm text-gray-500 leading-relaxed">
                {{ $tukang->bio ?? 'Tukang berpengalaman dengan keahlian di bidang perbaikan rumah, instalasi, dan berbagai layanan teknis lainnya.' }}
            </p>
        </div>
    </div>
    ...
    <div class="px-4 mt-6 pb-28">
        ...
        <div class="mb-4">
            ...
            <div id="layananPicker" class="flex flex-wrap gap-2">
                ...
                <label class="cursor-pointer layanan-chip">
                    <input type="radio" name="id_layanan_sel" value="{{ $l->id_layanan }}" class="hidden"
                           {{ $selectedLayanan == $l->id_layanan ? 'checked' : '' }}>
                    <span class="inline-block px-3 py-1.5 rounded-full text-xs font-semibold border-2 transition-all
                        {{ $selectedLayanan == $l->id_layanan ? 'bg-blue-600 border-blue-600 text-white' : 'border-gray-200 text-gray-500 bg-white' }}">
                        {{ $l->nama_layanan }}
                    </span>
                </label>
                @endforeach
        ...
        <form action="{{ route('orders.store') }}" method="POST" id="orderForm">
            @csrf
            <input type="hidden" name="id_tukang" value="{{ $tukang->id_tukang }}">
            <input type="hidden" name="id_layanan" id="selectedLayanan" value="{{ $selectedLayanan }}">
            <button type="submit"
                class="w-full bg-blue-600 text-white font-bold py-4 rounded-2xl text-base shadow-lg">
                Pesan Sekarang
            </button>
        </form>
        <a href="{{ route('chat.show', $tukang->id_tukang) }}"
