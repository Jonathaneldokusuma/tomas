...
<div style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden;margin-bottom:16px">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid #f1f5f9;gap:12px;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:0;border:1px solid #e2e8f0;border-radius:9px;overflow:hidden">
            <a href="{{ route('admin.tukang') }}" style="padding:7px 16px;font-size:12px;font-weight:600;background:{{ !request('status') ? '#2563eb':'#fff' }};color:{{ !request('status') ? '#fff':'#6b7280' }};text-decoration:none;white-space:nowrap">All Services</a>
            <a href="{{ route('admin.tukang') }}?status=1" style="padding:7px 16px;font-size:12px;font-weight:500;background:{{ request('status')==='1' ? '#2563eb':'#fff' }};color:{{ request('status')==='1' ? '#fff':'#6b7280' }};text-decoration:none;border-left:1px solid #e2e8f0;white-space:nowrap">Active</a>
            <a href="{{ route('admin.tukang') }}?status=0" style="padding:7px 16px;font-size:12px;font-weight:500;background:{{ request('status')==='0' ? '#2563eb':'#fff' }};color:{{ request('status')==='0' ? '#fff':'#6b7280' }};text-decoration:none;border-left:1px solid #e2e8f0;white-space:nowrap">Nonaktif</a>
        </div>
        <form method="GET" action="{{ route('admin.tukang') }}" style="display:flex;align-items:center;gap:8px">
            ...
            @if($q)<a href="{{ route('admin.tukang') }}" style="font-size:12px;color:#9ca3af;text-decoration:none">Reset</a>@endif
        </form>
    </div>
                ...
                <tr style="border-top:1px solid #f1f5f9" onmouseover="this.style.background='#fafbfc'" onmouseout="this.style.background=''">
                    <td style="padding:13px 18px">
                        <div style="display:flex;align-items:center;gap:10px">
                            @if($tukang->foto)
                            <img src="{{ Storage::url($tukang->foto) }}" alt="" style="width:34px;height:34px;border-radius:9px;object-fit:cover;flex-shrink:0;border:1px solid #e2e8f0">
                            @else
                            <div style="width:34px;height:34px;border-radius:9px;background:{{ $bg }};color:{{ $tx }};display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;flex-shrink:0">
                                {{ strtoupper(substr($tukang->nama,0,1)) }}
                            </div>
                            @endif
                            <div>
                                <div style="font-size:13px;font-weight:600;color:#111827">{{ $tukang->nama }}</div>
                                @if($tukang->bio)
                                <div style="font-size:11px;color:#9ca3af;max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ Str::limit($tukang->bio,45) }}</div>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td style="padding:13px 18px">
                        <span style="background:#eff6ff;color:#2563eb;font-size:11px;font-weight:600;padding:3px 10px;border-radius:20px;white-space:nowrap">{{ $tukang->kategori ?? '-' }}</span>
                    </td>
                    <td style="padding:13px 18px;font-size:12px;color:#6b7280;white-space:nowrap">{{ $tukang->lokasi ?? '-' }}</td>
                    ...
                    <td style="padding:13px 18px">
                        @if($tukang->updated_at)
                        <div style="font-size:11px;color:#374151;font-weight:500">{{ $tukang->updated_at->format('d M Y') }}</div>
                        <div style="font-size:10px;color:#9ca3af">{{ $tukang->updated_at->format('H:i') }}</div>
                        @else
                    ...
                    <td style="padding:13px 18px">
                        <div style="display:flex;align-items:center;gap:6px">
                            <a href="{{ route('admin.tukang.edit', $tukang->id_tukang) }}"
                            ...
                            <form action="{{ route('admin.tukang.delete', $tukang->id_tukang) }}" method="POST" style="display:inline"
                                  onsubmit="return confirm('Hapus {{ addslashes($tukang->nama) }}?')">
                                @csrf @method('DELETE')
                                <button type="submit"
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px">
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:16px 18px">
        ...
        <div style="font-size:24px;font-weight:800;color:#0d1b2e">{{ $list->total() }}</div>
    ...
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:16px 18px">
        ...
        <div style="font-size:24px;font-weight:800;color:#0d1b2e">{{ \App\Models\Tukang::where('status_aktif',1)->count() }}</div>
    ...
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:16px 18px">
        ...
        <div style="font-size:24px;font-weight:800;color:#0d1b2e">{{ \App\Models\Layanan::count() }}</div>
