...
<div style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid #f1f5f9;gap:12px;flex-wrap:wrap">
        ...
        <form method="GET" action="{{ route('admin.orders') }}" style="display:flex;align-items:center;gap:8px">
            <div style="position:relative">
                <i class="fas fa-search" style="position:absolute;left:9px;top:50%;transform:translateY(-50%);color:#9ca3af;font-size:10px"></i>
                <input type="text" name="q" value="{{ $q }}" placeholder="Cari user / tukang..."
            ...
            @if($q)<a href="{{ route('admin.orders') }}" style="font-size:12px;color:#9ca3af;text-decoration:none">Reset</a>@endif
        </form>
    </div>
        ...
        <table style="width:100%;border-collapse:collapse">
            ...
            <tbody>
                ...
                <tr style="border-top:1px solid #f1f5f9" onmouseover="this.style.background='#fafbfc'" onmouseout="this.style.background=''">
                    <td style="padding:13px 18px;font-family:monospace;font-size:12px;color:#374151;white-space:nowrap">#ORD-{{ str_pad($order->id_order,5,'0',STR_PAD_LEFT) }}</td>
                    <td style="padding:13px 18px">
                        <div style="display:flex;align-items:center;gap:9px">
                            <div style="width:30px;height:30px;border-radius:50%;background:{{ $bg }};color:{{ $tx }};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;flex-shrink:0">{{ strtoupper(substr($order->user->nama??'U',0,1)) }}</div>
                            <div>
                                <div style="font-size:13px;font-weight:600;color:#111827">{{ $order->user->nama ?? '-' }}</div>
                                <div style="font-size:11px;color:#9ca3af">{{ $order->user->no_hp ?? '' }}</div>
                            </div>
                        </div>
                    </td>
                    <td style="padding:13px 18px">
                        <span style="background:#eff6ff;color:#2563eb;font-size:11px;font-weight:600;padding:3px 10px;border-radius:20px;white-space:nowrap">{{ $order->layanan->nama_layanan ?? '-' }}</span>
                    </td>
                    <td style="padding:13px 18px">
                        <div style="font-size:13px;font-weight:500;color:#374151">{{ $order->tukang->nama ?? '-' }}</div>
                        <div style="font-size:11px;color:#9ca3af">{{ $order->tukang->kategori ?? '' }}</div>
                    </td>
                    <td style="padding:13px 18px">
                        @if($order->review)
                    ...
                    <td style="padding:13px 18px">
                        <form action="{{ route('admin.orders.delete', $order->id_order) }}" method="POST" style="display:inline"
                              onsubmit="return confirm('Hapus order #{{ $order->id_order }}?')">
                            @csrf @method('DELETE')
                            <button type="submit"
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 18px;border-top:1px solid #f1f5f9">
        <span style="font-size:12px;color:#6b7280">Showing {{ $orders->firstItem() }}–{{ $orders->lastItem() }} of {{ $orders->total() }} results</span>
