...
<div class="page-content bg-gray-50">
    ...
    <div class="flex items-center gap-3 px-4 pt-5 pb-3 bg-white border-b border-gray-100 sticky top-0 z-10">
        <a href="{{ url()->previous() }}" class="w-9 h-9 rounded-full flex items-center justify-center bg-gray-100">
    ...
    <div class="px-4 pt-4 pb-24 space-y-2">
        ...
        @endphp
        <div class="bg-white rounded-2xl p-4 shadow-sm border {{ $notif->dibaca ? 'border-gray-100' : 'border-blue-200' }} flex items-start gap-3">
            <div class="w-10 h-10 rounded-full {{ $style['bg'] }} flex items-center justify-center flex-none">
                <i class="{{ $style['icon'] }} {{ $style['color'] }} text-base"></i>
            </div>
            <div class="flex-1 min-w-0">
                <div class="flex items-start justify-between gap-2">
                    <p class="font-semibold text-navy text-sm leading-snug">{{ $notif->judul }}</p>
                    @if(!$notif->dibaca)
