...
@section('title', 'Support Center Tukang')
        ...
        <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;box-shadow:0 1px 6px rgba(0,0,0,.05);overflow:hidden">
            ...
            <div style="max-height:70vh;overflow:auto">
                @forelse($threads as $thread)
                    @php
                        $isActive = (string) $thread['id_tukang'] === (string) $selectedTukangId;
                        $name = $thread['tukang']->nama ?? 'Tukang';
                        $snippet = \Illuminate\Support\Str::limit($thread['last_message'] ?? '', 70);
                    @endphp
                    <a href="{{ route('admin.support', ['tukang_id' => $thread['id_tukang']]) }}"
        ...
        <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;box-shadow:0 1px 6px rgba(0,0,0,.05);overflow:hidden;display:flex;flex-direction:column;min-height:70vh">
            <div style="padding:16px 18px;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">
                <div>
                    <h2 style="font-size:15px;font-weight:700;color:#1e293b">
                        {{ $selectedTukang ? $selectedTukang->nama : 'Pilih thread di kiri' }}
                    </h2>
                    <p style="font-size:12px;color:#64748b;margin-top:2px">
                        @if($selectedTukang)
                            {{ $selectedTukang->kategori ?? 'Kategori belum diisi' }} · ID {{ $selectedTukang->id_tukang }}
                        @else
                ...
                @if(!$selectedTukangId)
                    <div style="height:100%;min-height:300px;display:flex;align-items:center;justify-content:center;color:#94a3b8;text-align:center">
                        <div>
                            ...
                            Pilih salah satu tukang untuk melihat percakapan support.
                        </div>
                    </div>
                @else
                    <div style="display:flex;flex-direction:column;gap:12px">
                        @forelse($messages as $message)
                            @php
                                $isTukang = (bool) $message->dari_tukang;
                            @endphp
                            <div style="display:flex;justify-content:{{ $isTukang ? 'flex-start' : 'flex-end' }}">
                                <div style="max-width:min(680px, 82%);background:{{ $isTukang ? '#fff' : '#dbeafe' }};border:1px solid {{ $isTukang ? '#e2e8f0' : '#bfdbfe' }};border-radius:16px;padding:12px 14px;box-shadow:0 1px 5px rgba(0,0,0,.04)">
                                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                                        <span style="font-size:11px;font-weight:700;color:{{ $isTukang ? '#2563eb' : '#1d4ed8' }}">
                                            {{ $isTukang ? 'Tukang' : 'Pusat' }}
                                        </span>
            ...
            @if($selectedTukangId)
                <div style="padding:16px 18px;border-top:1px solid #f1f5f9;background:#fff">
                    <form action="{{ route('admin.support.reply', $selectedTukangId) }}" method="POST" style="display:flex;flex-direction:column;gap:10px">
                        @csrf
                        <label style="font-size:12px;font-weight:600;color:#374151">Balas ke tukang</label>
